...
public class MXParser implements XmlPullParser {
    ...
    private static boolean[] lookupNameStartChar = new boolean[1024];
    private static boolean[] lookupNameChar = new boolean[1024];
    ...
    public MXParser() {
        this.bufferLoadFactor = (float)this.bufLoadFactor / 100.0F;
        this.buf = new char[Runtime.getRuntime().freeMemory() > 1000000L ? 8192 : 256];
        this.bufSoftLimit = (int)(this.bufferLoadFactor * (float)this.buf.length) / 100;
    ...
    public boolean getFeature(String name) {
        if (name == null) {
            ...
            return FEATURE_XML_ROUNDTRIP.equals(name) ? this.roundtripSupported : false;
        }
    }
    ...
    public Object getProperty(String name) {
        if (name == null) {
            ...
            return PROPERTY_LOCATION.equals(name) ? this.location : null;
        }
    }
    ...
    public int parseEndTag() throws XmlPullParserException, IOException {
        ...
        if (!this.isNameStartChar(ch)) {
            ...
            if (this.elRawNameEnd[this.depth] != len) {
                ...
                throw new XmlPullParserException("end tag name </" + endname + "> must match start tag name <" + startname + "> from line " + this.elRawNameLine[this.depth], this, (Throwable)null);
            } else {
                for(int i = 0; i < len; ++i) {
                ...
                if (!this.isNameChar(ch)) {
                    ...
                    if (this.processNamespaces) {
                        ...
                        for(int i = 1; i < this.attributeCount; ++i) {
                            for(int j = 0; j < i; ++j) {
                                if (Objects.equals(this.attributeUri[j], this.attributeUri[i]) && (this.allStringsInterned && this.attributeName[j].equals(this.attributeName[i]) || !this.allStringsInterned && this.attributeNameHash[j] == this.attributeNameHash[i] && this.attributeName[j].equals(this.attributeName[i]))) {
                        ...
                        for(int i = 1; i < this.attributeCount; ++i) {
                            for(int j = 0; j < i; ++j) {
                                if (this.allStringsInterned && this.attributeName[j].equals(this.attributeName[i]) || !this.allStringsInterned && this.attributeNameHash[j] == this.attributeNameHash[i] && this.attributeName[j].equals(this.attributeName[i])) {
                                    attrPrefix = this.attributeName[j];
        ...
        if (ch == '#') {
            ...
            if (!noUnicode4) {
                try {
                    ...
                    throw new XmlPullParserException("character reference (with " + (isHex ? "hex" : "decimal") + " value " + sb.toString() + ") is invalid", this, (Throwable)null);
            ...
            if (noUnicode4) {
                ...
                if (i > 65535) {
                    throw new XmlPullParserException("Unicode character reference (with " + (isHex ? "hex" : "decimal") + " value " + sb.toString() + ") is not supported in this runtime", this, (Throwable)null);
                }
    ...
    private char[] lookuEntityReplacement(int entityNameLen) throws XmlPullParserException, IOException {
                ...
                if ((hash == this.entityNameHash[i]) && (entityNameLen == this.entityNameBuf[i].length)) {
                    ...
                    for (int j = 0; j < entityNameLen; j++)
                    {
                        if (this.buf[(this.posStart + j)] != entityBuf[j])
                            break label130; }
                    if (this.tokenize) this.text = this.entityReplacement[i];
    ...
    private boolean parsePI() throws XmlPullParserException, IOException {
        ...
        try {
            ...
            while(true) {
                ...
                if (ch == '?') {
                    if (!seenPITarget) {
                        throw new XmlPullParserException("processing instruction PITarget name not found", this, (Throwable)null);
                    }
                    ...
                    if (!seenInnerTag) {
                        throw new XmlPullParserException("processing instruction started on line " + curLine + " and column " + curColumn + " was not closed", this, (Throwable)null);
                    }
                    ...
                    if (piTargetEnd == -1 && this.isS(ch)) {
                        ...
                        if (piTargetEnd - piTargetStart == 3 && (this.buf[piTargetStart] == 'x' || this.buf[piTargetStart] == 'X') && (this.buf[piTargetStart + 1] == 'm' || this.buf[piTargetStart + 1] == 'M') && (this.buf[piTargetStart + 2] == 'l' || this.buf[piTargetStart + 2] == 'L')) {
                            if (piTargetStart > 3) {
                                throw new XmlPullParserException("processing instruction can not have PITarget with reserved xml name", this, (Throwable)null);
                            }
            ...
            throw new XmlPullParserException("processing instruction started on line " + curLine + " and column " + curColumn + " was not closed", this, var13);
        }
        ...
        if (this.reader == null) {
            ...
            if (this.bufEnd > this.bufSoftLimit) {
                ...
                if (compact) {
                    ...
                    ret = 2 * this.buf.length;
                    char[] newBuf = new char[ret];
    ...
    private boolean isNameStartChar(char ch) {
        return ch < 1024 ? lookupNameStartChar[ch] : ch <= 8231 || ch >= 8234 && ch <= 8591 || ch >= 10240 && ch <= '\uffef';
    }

    private boolean isNameChar(char ch) {
        return ch < 1024 ? lookupNameChar[ch] : ch <= 8231 || ch >= 8234 && ch <= 8591 || ch >= 10240 && ch <= '\uffef';
    }
